...
export default class UpdateGroup extends React.Component {
  ...
  renderButton = () => {
    ...
    if (this.state.disabled) {
      return (
        <div>
          <button className="btn-arb" style={{ "padding": "4px" }} disabled>{words.institution_update_group}</button>
          <button className="btn-arb red-i" style={{ "padding": "4px" }} onClick={(e) => this.props.cancel(e)}>{words.institution_cancel_update_group}</button>
        </div>
      )
      ...
      return (
        <div>
          <button className="btn-arb" id="update" style={{ "padding": "4px" }} onClick={this.updateHandle}>{words.institution_update_group}</button>
          <button className="btn-arb red-i" style={{ "padding": "4px" }} onClick={(e) => this.props.cancel(e)}>{words.institution_cancel_update_group}</button>
        </div>
      )
    }
