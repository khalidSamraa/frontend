  ...
  renderMenu() {
    ...
    return (
      <div className="dropdown-content">
        ...
        <a tabIndex="0" role="button" onClick={this.groupRightsClick}>{words.institution_group_rights}</a>
        {this.state.in_update ?
          <a tabIndex="0" role="button" className="red-i" onClick={this.cancel}>{words.institution_cancel_rename}</a>
          :
          <a tabIndex="0" role="button" onClick={this.setTarget}>{words.institution_rename_group}</a>
        }
        <a tabIndex="0" role="button" className="red-i" onClick={this.removeGroup}>{words.institution_delete_group}</a>
      </div>
    )
  }

  render() {
    const {rowData, active, countMember, InstitutionReducer, words} = this.props
