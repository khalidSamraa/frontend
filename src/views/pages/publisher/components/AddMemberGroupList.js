    ...
    var element = row.map((val, index) => {
      return (
        <AddMemberGroupRow
          ...
          index={index}
          PublisherReducer={this.props.PublisherReducer}
          words={words}
        />
      )
    })
  ...
  renderMenu() {
    ...
    return (
      <div className="dropdown-content">
        ...
        <a tabIndex="0" role="button" onClick={() => this.props.setMode("add")}>{words.publisher_add_member}</a>
      </div>
    )
  }
    ...
    return (
      <div className={classnames("hide", { "show": show })}>
        <div className="gi-header">
          <div className="gi-title">{words.publisher_members}</div>
        ...
        {mode === "add" ?
          <div className="add-member-to-group">
            {/*<span>{words.publisher_add_member_to_group} <a className="gname">{groupName}</a></span> */}
            <span>{words.publisher_add_member_to_group}</span>
          ...
          <div className="col-xs-12">
            <div className="box">
              <UserSearch
                ...
                placeholder={words.publisher_field_member_name}
                toggle={this.state.toggleSearch}
                rtl={true}
              />
              <ul className={classnames("member-list scrolled-item w99")} onScroll={this.handleScroll}>
                <Animated total={5} loading={isLoading} error={isError} count={count} text={words.publisher_member_empty} animation={<UserList />}>
                  {rowElement}
                </Animated>
