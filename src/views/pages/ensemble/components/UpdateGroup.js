...
export default class UpdateGroup extends React.Component {
  ...
  renderButton = () => {
    ...
    if (this.state.disabled) {
      return (
        <div>
          <button className="btn-arb" style={{ "padding": "4px" }} disabled>{words.ensemble_button_update_group}</button>
          <button className="btn-arb red-i" style={{ "padding": "4px" }} onClick={(e) => this.props.cancel(e)}>{words.ensemble_button_cancel_group}</button>
        </div>
      )
      ...
      return (
        <div>
          <button className="btn-arb" id="update" style={{ "padding": "4px" }} onClick={this.updateHandle}>{words.ensemble_button_update_group}</button>
          <button className="btn-arb red-i" style={{ "padding": "4px" }} onClick={(e) => this.props.cancel(e)}>{words.ensemble_button_cancel_group}</button>
        </div>
      )
    }
