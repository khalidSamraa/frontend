    ...
    return (
      <div className="dropdown-content">
        ...
        {this.state.in_update ?
          <a tabIndex="0" role="button" className="red-i" onClick={this.cancel}>{words.ensemble_cancel_rename}</a>
          :
          <a tabIndex="0" role="button" onClick={this.setTarget}>{words.ensemble_rename_group}</a>
        }
        <a tabIndex="0" role="button" className="red-i" onClick={this.removeGroup}>{words.ensemble_delete_group}</a>
      </div>
    )
  }

  render() {
    const { rowData, active, countMember, words } = this.props
